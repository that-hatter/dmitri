...
const elemEmbed = (element: table.Element) => {
  ...
  const rsclink = "https://www.rsc.org/periodic-table/element/" + fetch("number");
  const pbclink = "https://pubchem.ncbi.nlm.nih.gov/element/" + fetch("number");
  const nistlink = "https://webbook.nist.gov/cgi/inchi/InChI%3D1S/" + fetch("symbol");
  return {
    embed: {
      ...
      description: 
        `**Mass**: ${ fetch("atomic_mass") } | **Phase at STP**: ${ fetch("phase") }\n`
